-- 016_university_program_enrichment.sql
-- Safe additive enrichment for university and program profile pages
...
ALTER TABLE universities
  ...
  CHECK (
    ...
    OR (founded_year >= 1000 AND founded_year <= EXTRACT(YEAR FROM NOW())::INT)
  );
...
ALTER TABLE universities
  ...
  CHECK (
    international_students_percent IS NULL
    OR (international_students_percent >= 0 AND international_students_percent <= 100)
  );
...
ALTER TABLE universities
  ...
  CHECK (
    female_percent IS NULL
    OR (female_percent >= 0 AND female_percent <= 100)
  );
...
ALTER TABLE universities
  ...
  CHECK (
    acceptance_rate_estimate IS NULL
    OR (acceptance_rate_estimate >= 0 AND acceptance_rate_estimate <= 100)
  );

CREATE INDEX IF NOT EXISTS idx_universities_slug
  ON universities(slug);

CREATE INDEX IF NOT EXISTS idx_universities_country_name
  ON universities(country_name);

CREATE INDEX IF NOT EXISTS idx_universities_region_or_state
  ON universities(region_or_state);

CREATE INDEX IF NOT EXISTS idx_universities_university_type
...
ALTER TABLE programs
  ...
  CHECK (
    ...
    OR (duration_months >= 1 AND duration_months <= 120)
  );

CREATE INDEX IF NOT EXISTS idx_programs_slug
  ON programs(slug);

CREATE INDEX IF NOT EXISTS idx_programs_subfield
  ON programs(subfield);

CREATE INDEX IF NOT EXISTS idx_programs_study_language
  ON programs(study_language);